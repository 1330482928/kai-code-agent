...
| `$OPENCODE_REPO/packages/opencode/src/session/processor.ts` | L499-L558 | usage、snapshot、compaction flag | `src/session/store.ts`, `src/context/manager.ts` |
| `$OPENCODE_REPO/packages/opencode/src/session/processor.ts` | L638-L794 | cleanup、halt、retry、overflow | `src/agent/recovery.ts` |
| `$OPENCODE_REPO/packages/opencode/src/session/llm.ts` | L76-L129 | provider/config/auth/prompt 合成 | `src/provider/index.ts`, `src/prompt/composer.ts` |
...
| `$OPENCODE_REPO/packages/opencode/src/tool/registry.ts` | L189-L202 | custom tool 加载 | Stage 13 可选扩展 |
...
| `$OPENCODE_REPO/packages/opencode/src/session/compaction.ts` | L122-L203 | summary prompt、turn splitting | `src/context/compaction.ts` |
| `$OPENCODE_REPO/packages/opencode/src/session/overflow.ts` | L6-L26 | usable context 计算 | `src/context/budget.ts` |
...
| `$OPENCODE_REPO/packages/opencode/src/session/instruction.ts` | L13-L163 | AGENTS/CLAUDE/CONTEXT loader | `src/prompt/instructions.ts` |
...
| `$CLAUDE_CODE_REPO/src/constants/prompts.ts` | L444-L577 | prompt 动态段合成 | `src/prompt/composer.ts` |
| `$CLAUDE_CODE_REPO/src/context.ts` | L36-L149 | git/system context | `src/prompt/context.ts` |
...
| `$CODEX_REPO/codex-rs/core/src/session/mcp.rs` | L68-L159 | MCP elicitation request/response | Stage 13 可选高级能力 |
...
| `prompt` | OpenCode instruction | Claude prompt composer | 静态原则 + 项目指令 + 动态上下文 |
| `context` | OpenCode compaction | Claude query abort cleanup | token budget + summary + tail preserve |
