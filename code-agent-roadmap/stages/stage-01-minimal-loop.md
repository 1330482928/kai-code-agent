# Stage 01: Minimal Real LLM Loop

## 1. 本阶段目标

本阶段完成最小真实 LLM 闭环：用户启动 `kai` 或执行 `kai run "<task>"`，CLI 先校验默认模型配置；如果没有模型配置，则进入首次配置向导，收集 provider preset、API key、model name 和必要的 baseURL，并保存到用户级配置文件。配置完成后，Agent 把任务包装成 message，调用真实 OpenAI-compatible LLM API，流式打印回答，并把一次 turn 的内存状态保留下来。
...
| vitest | loop、provider、config、CLI smoke 测试 |
...
| 输出 | async stream | async generator | event stream | async iterable；参考 §4 源码引用 | 能统一真实 API streaming 和 fixture replay。 |
...
flowchart LR
  ...
  Config --> Wizard["src/config/first-run.ts"]
  Config --> Factory["src/provider/factory.ts"]
  CLI --> Loop["src/agent/loop.ts"]
  Loop --> Msg["src/agent/messages.ts"]
  Msg --> Provider["src/provider/openai.ts"]
  Factory --> Provider
  Provider --> UI["src/ui/render.ts"]
  UI --> User
  Fixture["src/provider/fixture.ts"] -. tests .-> Loop
...
| `src/config/first-run.ts` | 首次配置向导和 provider preset | ~140 | `ensureModelConfig` |
| `src/agent/messages.ts` | 定义 Message、Role、RunResult | ~70 | `Message` |
| `src/agent/loop.ts` | `runOnce()`，调用 provider，收集输出 | ~130 | `AgentLoop` |
| `src/provider/types.ts` | provider 输入、输出、错误接口 | ~80 | `ProviderAdapter`, `ProviderEvent` |
| `src/provider/openai.ts` | OpenAI-compatible streaming adapter | ~180 | `OpenAIProvider` |
| `src/provider/factory.ts` | 根据 config 创建 provider adapter | ~60 | `createProvider` |
| `src/provider/fixture.ts` | 测试用 fixture event replay | ~70 | `FixtureProvider` |
| `src/ui/render.ts` | 输出文本和 provider 错误摘要 | ~40 | `render` |
...
6. OpenAI-compatible provider 调用真实 API，转换 streaming event 为内部 `ProviderEvent`。
7. renderer 把 `text_delta` 写到 stdout。
...
5. 实现 OpenAI-compatible provider 的最小 streaming text adapter。
6. 实现 fixture provider，用 fixture replay 支撑单测和 CI。
7. 写 CLI 命令：`kai`、`kai run "<task>"`、`kai config show`。
8. 增加 config、provider adapter、loop、CLI smoke test。
...
pnpm install
pnpm kai
pnpm kai run "summarize this repo"
pnpm kai run --provider fixture --script fixtures/provider/basic-text.json "hello"
pnpm test -- stage-01
```
...
| 验收项 | 标准 |
| --- | --- |
| 首次配置 | 无 `~/.kai-code-agent/config.yaml` 时，`pnpm kai` 进入配置向导并保存配置 |
| 配置安全 | 用户级配置不在项目目录内，写入权限为 `0600` |
| 真实 API 可跑 | 配置有效 API key 后，`pnpm kai run "hello"` 能流式打印 assistant 文本 |
